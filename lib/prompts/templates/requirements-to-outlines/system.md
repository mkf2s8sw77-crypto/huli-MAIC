...
6. `interactive` scenes should include `widgetType` and `widgetOutline` whenever the concept truly benefits from hands-on interaction. `interactiveConfig` is deprecated and only accepted for backwards compatibility.
7. `pbl` scenes must include `pblConfig` with `projectTopic`, `projectDescription`, `targetSkills`, `issueCount`.
8. Arrange scenes by inferred duration and orientation: landscape typically 1-2 scenes per minute; portrait typically 1.5-2.5 scenes per minute, with more focused scenes and less per scene.
